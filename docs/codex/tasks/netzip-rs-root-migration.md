...
- 目标：将 `netzipapi-rust-demo` 独立仓库扁平化到 `/home/codes/quoteNetzipRs`，使该目录成为唯一工程根。
...
- `/home/codes/quoteNetzipRs/.git` 存在，旧 `netzipapi-rust-demo/` 不再作为源码入口。
- 构建清单、部署脚本和文档不再依赖原 `netzipapi-rust-demo/` 嵌套源码入口。
- `cargo metadata --no-deps` 成功，迁移后的 Git 差异边界可审计。

## 风险与未完成项

- webClx 已用项目标识 `quoteNetzipRs`、工作目录 `/home/codes/quoteNetzipRs` 完成 release 编译；请求 `162028-18c8f2f31ead11cc`。
