...
unit_source="$project_dir/deploy/quote-netzip-rs.service"
unit_target="/etc/systemd/system/quote-netzip-rs.service"
...
systemctl enable quote-netzip-rs.service
systemctl restart quote-netzip-rs.service
...
systemctl is-active --quiet quote-netzip-rs.service
if [[ "${NETZIP_INSTALL_ENABLE_FULL_PUSH:-1}" == "1" ]]; then
